...
Plugin Name: Vimeo.com
Plugin URI: http://www.lukemcdonald.com/
Description: Add your Vimeo.com feed
...
add_action( 'widgets_init', 'hoon_vimeo_widget_load' );

function hoon_vimeo_widget_load() { 
	register_widget( 'hoon_vimeo_widget' ); // register the widget
} 

class hoon_vimeo_widget extends WP_Widget {
	
	function hoon_vimeo_widget() {
		// define widget settings
		$widget_ops = array( 
			'classname' => 'hoon_vimeo_widget', 
			'description' => __( 'Add Your Vimeo.com Feed', 'hoon' ) 
		);
		...
		$control_ops = array( 
			'width' => 250, 
			'height' => 250, 
			'id_base' => 'hoon_vimeo_widget' 
		);
		
		// create the widget
		$this->WP_Widget( 'hoon_vimeo_widget', __( 'Vimeo.com', 'hoon' ), $widget_ops, $control_ops ); 
	}
	
    function form( $instance ) {
	    $defaults = array(
	    	'title' => __( 'My Latest Videos via Vimeo.com:', 'hoon' ),
	    	'username' => '',
	    	'count' => 6
	    );
    ...
    function widget( $args, $instance ) {
		...
		print do_shortcode( sprintf( '[p75_vimeo username="%1$s" count="%2$s"]', 
